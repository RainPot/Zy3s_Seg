...
class PAmodule(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4, x0):  #x1:256  x2:512  x3:1024  x4:2048  x0:128
        ...
        x1_down = F.interpolate(x1_down, size=x4_down.size()[2:], mode='bilinear', align_corners=True)
        x2_down = F.interpolate(x2_down, size=x4_down.size()[2:], mode='bilinear', align_corners=True)
        x0_up = F.interpolate(x0_up, size=x4_down.size()[2:], mode='bilinear', align_corners=True)
...
class DIGModule(nn.Module):
    ...
    def forward(self, feat, stagecur, stage0):
        ...
        stage0 = F.interpolate(stage0, size=feat_stagecur_cat.size()[2:], mode='bilinear', align_corners=True)
