...
class Kernel_Representation(nn.Module):
    def __init__(self):
        ...
        self.x1_order3_1 = Kernel_Calculate(256, 384)
        self.x1_order3_2 = Kernel_Calculate(256, 384)
        self.x1_order3_3 = Kernel_Calculate(256, 384)

        self.x2_order2_1 = Kernel_Calculate(512, 384)
        self.x2_order2_2 = Kernel_Calculate(512, 384)

        self.x3_order1_1 = Kernel_Calculate(1024, 384)

        self.x4_order3_1 = Kernel_Calculate(2048, 384)
        self.x4_order3_2 = Kernel_Calculate(2048, 384)
        self.x4_order3_3 = Kernel_Calculate(2048, 384)
        self.x4_final = Kernel_Calculate(384, 768)

        self.x1_x4_gate = ConvBNReLU(768, 768, kernel_size=1, stride=1, padding=0)
        self.x2_x4_gate = ConvBNReLU(768, 768, kernel_size=1, stride=1, padding=0)
        self.x3_x4_gate = ConvBNReLU(768, 768, kernel_size=1, stride=1, padding=0)
        self.x4_gate = ConvBNReLU(768, 768, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x1, x2, x3, x4):
        x1_order3_1 = self.x1_order3_1(x1)
        x1_order3_2 = self.x1_order3_2(x1)
        ...
        x1_x4_gate = self.x1_x4_gate(x1_x4)
        x1_x4_gate = x1_x4 * x1_x4_gate
        x1_x4 = x1_x4 + x1_x4_gate


        x2_x4 = torch.cat((x4, x2), dim=1)
        x2_x4_gate = self.x2_x4_gate(x2_x4)
        x2_x4_gate = x2_x4 * x2_x4_gate
        x2_x4 = x2_x4 + x2_x4_gate

        x3_x4 = torch.cat((x4, x3), dim=1)
        x3_x4_gate = self.x3_x4_gate(x3_x4)
        x3_x4_gate = x3_x4 * x3_x4_gate
        x3_x4 = x3_x4 + x3_x4_gate

        x4 = self.x4_final(x4)
        x4_gate = self.x4_gate(x4)
        x4_gate = x4 * x4_gate
        x4 = x4 + x4_gate
...
class Feature_Fusion(nn.Module):
    def __init__(self):
        ...
        self.featurefusion1 = ConvBNReLU(768, 256, padding=18, dilation=18)
        self.featurefusion2 = ConvBNReLU(768, 256, padding=12, dilation=12)
        self.featurefusion3 = ConvBNReLU(768, 256, padding=6, dilation=6)
        self.featurefusion4 = ConvBNReLU(768, 256, padding=1, dilation=1)
