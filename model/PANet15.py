...
class PAmodule(nn.Module):
    def __init__(self):
        ...
        self.x4_down_conv = ConvBNReLU(2048, 384, kernel_size=1, stride=1, padding=0)
        self.x0_up_conv = ConvBNReLU(128, 128, kernel_size=1, stride=1, padding=0)
        ...
        self.x024conv = ConvBNReLU(512, 256, kernel_size=3, stride=1, padding=12, dilation=12)
        self.x034conv = ConvBNReLU(512, 256, kernel_size=3, stride=1, padding=10, dilation=10)
        self.x124conv = ConvBNReLU(512, 256, kernel_size=3, stride=1, padding=9, dilation=9)
        self.x134conv = ConvBNReLU(512, 256, kernel_size=3, stride=1, padding=6, dilation=6)

        self.avg = nn.AdaptiveAvgPool2d((1, 1))
        self.GAPConv = ConvBNReLU(2048, 256, kernel_size=1, stride=1, padding=0)

        self.feature_fusion = ConvBNReLU(256 * 7, 256, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x1, x2, x3, x4, x0):  #x1:256  x2:512  x3:1024  x4:2048  x0:128
        ...
        x024 = self.x024conv(torch.cat((x4x0, x4x2), dim=1))
        x034 = self.x034conv(torch.cat((x4x0, x4x3), dim=1))
        x124 = self.x124conv(torch.cat((x4x1, x4x2), dim=1))
        x134 = self.x134conv(torch.cat((x4x1, x4x3), dim=1))


        feat = torch.cat((x014, x234, x024, x034, x124, x134, x4GAP), dim=1)
